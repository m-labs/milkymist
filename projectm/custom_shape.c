...
custom_shape_t * new_custom_shape(int id) {
  ...
  if (insert_param(param, custom_shape->param_tree) < 0) {
   ...
   if ((param = new_param_double("rad", P_FLAG_NONE, &custom_shape->radius, NULL, MAX_DOUBLE_SIZE, 0, 0.0)) == NULL) {
    free_custom_shape(custom_shape);
    return NULL;
  }
...
inline void evalCustomShapeInitConditions() {
  splay_traverse(eval_custom_shape_init_conds, PM->active_preset->custom_shape_tree);
...
inline custom_shape_t * nextCustomShape() {
  ...
  if ((interface_shape = splay_find(&cwave_interface_id, PM->active_preset->custom_shape_tree)) == NULL) {
