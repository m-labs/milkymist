...
custom_wave_t * new_custom_wave(int id) {
  ...
  custom_wave->r_mesh = wipemalloc(MAX_SAMPLE_SIZE*sizeof(double));
  custom_wave->g_mesh = wipemalloc(MAX_SAMPLE_SIZE*sizeof(double));
  custom_wave->b_mesh = wipemalloc(MAX_SAMPLE_SIZE*sizeof(double));
  custom_wave->a_mesh = wipemalloc(MAX_SAMPLE_SIZE*sizeof(double));
  custom_wave->x_mesh = wipemalloc(MAX_SAMPLE_SIZE*sizeof(double));
  custom_wave->y_mesh = wipemalloc(MAX_SAMPLE_SIZE*sizeof(double));
  custom_wave->value1 = wipemalloc(MAX_SAMPLE_SIZE*sizeof(double));
  custom_wave->value2 = wipemalloc(MAX_SAMPLE_SIZE*sizeof(double));
  custom_wave->sample_mesh = wipemalloc(MAX_SAMPLE_SIZE*sizeof(double));
...
inline void evalCustomWaveInitConditions() {
  splay_traverse(eval_custom_wave_init_conds, PM->active_preset->custom_wave_tree);
}
...
inline custom_wave_t * nextCustomWave() {
  ...
  if ((interface_wave = splay_find(&interface_id, PM->active_preset->custom_wave_tree)) == NULL) {
    interface_id = 0;
    return NULL;
  }
...
inline void evalPerPointEqn(per_point_eqn_t * per_point_eqn) {
  ...
  if (per_point_eqn->param->matrix == NULL) {
    if ((param_matrix = per_point_eqn->param->matrix = wipemalloc(size = samples*sizeof(double))) == NULL)
      return;
