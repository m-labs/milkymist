...
inline void evalPerPixelEqn(per_pixel_eqn_t * per_pixel_eqn) {
 ...
 if (per_pixel_eqn->param->matrix == NULL) {
    ...
    param_matrix = (double**)wipemalloc(PM->gx*sizeof(double*));
    per_pixel_eqn->param->matrix = param_matrix;

    for(x = 0; x < PM->gx; x++)
      param_matrix[x] = (double *)wipemalloc(PM->gy * sizeof(double));
...
inline void evalPerPixelEqns() {
  ...
  splay_traverse(evalPerPixelEqn, PM->active_preset->per_pixel_eqn_tree);
...
inline int isPerPixelEqn(int op) {
  ...
  return PM->active_preset->per_pixel_flag[op];
