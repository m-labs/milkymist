...
However, there are some differences between the LatticeMico32 configuration and the Nios II/f configuration used in the Altera report:
\begin{itemize}
...
\paragraph{Characterization of cache conflicts.} A pixel at address $a_{p}$ (measured in pixels since the start of the framebuffer) is mapped to the cache line indexed by:
\begin{equation}
...
\begin{itemize}
\item 1 pixel in the most common case (sampling in the middle of the texture). This cannot cause cache conflicts.
\item $H-1$ pixels if texture wrapping is enabled and the texture is sampled at one of its vertical edges. In this case, the condition $|a_{A}-a_{B}| > N_{l}$ is often verified (except for small textures where $H < N_{l}$). To make sure that there will be no ICCC, we must thus make sure that the following condition is also verified:
...
\left\lfloor \frac{H-1}{N_{l}} \right\rfloor + 1 \not \equiv 0 \pmod{\frac{N_{c}}{N_{l}}}
\end{cases}
}
\end{equation}

There is another way to make sure that the equation~\ref{eq:noconflictwrap1} is always verified: by imposing alignment requirements. We make the following two hypotheses:
\begin{itemize}
\item The beginning of the framebuffer is aligned to the beginning of a cache line.
\item The horizontal texture resolution is a multiple of the cache line length in pixels.
\end{itemize}
Without loss of generality, we suppose that the pixel at the address $a_{A}$ is at the beginning of the texture line (its X coordinate is 0) -- and the one at the address $a_{B}$ is at the end of the line (its X coordinate is $H-1$). Along with equation~\ref{eq:fbadr}, this implies that $a_{A}$ is a multiple of $N_{l}$. The equation~\ref{eq:noconflictwrap1} can thus be simplified to:
\begin{equation}
\boxed{
\left\lfloor \frac{H-1}{N_{l}} \right\rfloor \not \equiv 0 \pmod{\frac{N_{c}}{N_{l}}}
}
\end{equation}
which removes one condition compared to the equation~\ref{eq:noconflictwrap2} which is valid in the general case (without alignment requirements).
\end{itemize}
...
The write buffer is in charge of gathering the final pixel data produced by the algorithm, assemble them into FML bursts and write them to the memory.
...
In Milkymist, the color format uses 16 bits per pixel and the FML bus is based on bursts of four 64-bit words. Therefore, the write buffer can tolerate write latencies of up to 12 cycles while maintaining excellent performance, which is achievable.
